...
export class AIService {
  ...
  static async summarizeNews(
    ...
    if (news.length === 0 && timeRange !== "MAX") {
      return {
    ...
    try {
      ...
      const jsonFormat = `반드시 아래 JSON 형식으로만 응답하세요 (마크다운 코드블록 없이):
{
  "headline": "25자 이내 헤드라인. '주가 상승/하락' 같은 추상적 표현 절대 금지",
  "summary": "2문장 이내의 간결한 종합 분석 (한국어)",
  "keyFactors": [
    "기업요인: 실적/경영/제품 등 (없으면 생략)",
    "거시요인: 금리/환율/경기 등 (없으면 생략)",
    "정치/외부: 정치·지정학·규제 등 (없으면 생략)"
  ],
  "sentiment": "positive 또는 negative 또는 neutral"
}
keyFactors는 실제로 해당하는 항목만 2~4개, 각 30자 이내로.`;

      let prompt: string;

      if (news.length === 0) {
        // MAX 뉴스 없음: AI 학습 데이터 기반 역사적 분석
        prompt = `당신은 한국어로 응답하는 전문 금융 애널리스트입니다.
${dateLabel} ${symbol} 주가의 해당 월 흐름을 당신의 금융 지식과 학습 데이터를 바탕으로 분석해 주세요.
${priceContext}
해당 시점의 거시경제 환경, 업종 동향, 기업 이슈를 근거로 설명하세요. 확실하지 않은 내용은 추측임을 표시하세요.
